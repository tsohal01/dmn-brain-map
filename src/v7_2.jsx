...
const CardHeader = ({ title, subtitle, accent = UI.color.textStrong, right }) => (
  <div
    style={{
      display: "flex",
      alignItems: "center",
...
const SubsystemCouplingConn = ({ pathD, status, delay = 0, dimmed = false }) => {
  if (!pathD) return null;

  const st = getSemanticEdgeStyle(status, 1.0);

  return (
    <g opacity={dimmed ? 0.16 : 1} style={{ transition: "opacity 0.35s ease" }}>
      {/* visible line — quieted to read as subsystem-level trend */}
      <path
        d={pathD}
        fill="none"
        stroke={st.color}
        strokeWidth={st.width}
        strokeDasharray={st.dash === "none" ? "10,0" : st.dash}
        strokeLinecap="round"
        pathLength="100"
        strokeDashoffset="100"
        opacity={status === "normal" ? 0.25 : 0.42}
        style={{
          animation: `subEdgeGrow 0.95s ease-out ${delay}s forwards`,
          transition: "all 0.25s ease",
        }}
      />
      {/* traveling dots removed — subsystem arcs should whisper, not shout */}
    </g>
  );
};
...
const HierarchyTab = () => {
  ...
  const [focusNode, setFocusNode] = useState(null); // node id or null
  ...
  const isSubZoomed = !!zoomedSub;       // only subsystem zoom dims other subsystems
  const isNodeFocus = !!focusNode;       // node focus is its own mode
  const isZoomed = isSubZoomed || isNodeFocus; // keep this if other UI needs “zoomed”
  const getZoomTransform = () => {
    ...
    // If a node is selected, zoom to THAT node (largest zoom)
    if (selectedNode && nodePos[selectedNode]) {
      const { cx: tcx, cy: tcy } = nodePos[selectedNode];
    
      const targetX = W * 0.76;
      const targetY = H * 0.46;
    
      const nodeTargetDiameter = 350; // increase this for more zoom
      const S = nodeTargetDiameter / (nodeR * 2);
  ...
  const handleBgClick = () => {
    setFocusNode(null);     // node focus off
    setZoomedSub(null);     // subsystem zoom off (if any)
    setSelectedNode(null);
    setHov(null);
  };
...
const MODE_EDGES = overlayTop === "state" ? MODE_EDGES_STATE : MODE_EDGES_TRAIT;
  ...
  const renderNode = (id, np, color, isRsp = false) => {
    ...
    return (
      <g key={id}>
        ...
        {/* Active pulse ring for mode-active nodes — single subtle ring, no spotGlow halo */}
  ...
  onClick={(e) => {
    ...
    const parentSub =
      ...
      SUB_NODES.aff.has(id) ? "aff" : null;
  
    // make sure you're inside the right subsystem first
    if (parentSub && zoomedSub !== parentSub) {
    ...
    // then zoom to node a fraction later so the spin is actually visible
    setTimeout(() => {
      setSelectedNode(id);
    }, 180);
  
    // clear spin state after animation ends
    setTimeout(() => {
...
>
  ...
  <g transform={`translate(${np.cx} ${np.cy})`}>
  <g
    key={`spinwrap-${id}-${spinTick}`}
    style={{
      transformBox: "fill-box",
      transformOrigin: "center",
      animation: spinningNode === id ? "nodeSpin 0.8s ease-in-out 1" : "none",
    }}
  >
   <circle
  cx={0}
...
>
  ...
  const hovNodeData = hov ? allNodes.find(n => n.id === hov) : null;
    ...
    return (
      <div style={{
        ...
        <button onClick={() => {setZoomedSub(null);setFocusNode(null);setHov(null);setSelectedNode(null);}}
            style={{
  ...
  <>
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 8 }}>
      <div style={{ width: 10, height: 10, borderRadius: "50%", background: selectedNodeData.color || zoomedColor, flexShrink: 0 }} />
      <span style={{ fontSize: 14, fontWeight: 700, color: selectedNodeData.color || "#D0E0F0", fontFamily: "'IBM Plex Sans', sans-serif" }}>
        {selectedNodeData.full || selectedNodeData.label}
      </span>
    </div>
    {selectedNodeData.full && (
    ...
    <GlassCard
      ...
      style={{
        marginTop: 8,
        padding: "10px 12px",
        borderLeft: `3px solid ${(selectedNodeData.color || zoomedColor) + "66"}`,
      }}
    >
      <div style={{ marginBottom: 7, display: "flex", gap: 6, flexWrap: "wrap" }}>
        <StatusChip label="Focused node" tone="verified" />
        <StatusChip label={(selectedNodeData.full || selectedNodeData.label).includes("Cortex") ? "Cortical region" : "Network region"} tone="neutral" />
      </div>
...
/>
  ...
  >
    <textPath
      href={`#arc-${subId}-txt`}
      textAnchor="middle"
      // IMPORTANT: keep this stable so it doesn't jump to bottom
      startOffset="50%"
    >
      {hov === subId ? (
        // "park" animation (tiny nudge, then freeze)
        <animate
          key={`${subId}-park`}
        ...
        // "spin" animation (remounts when hover ends)
        <animate
          key={`${subId}-spin`}
          attributeName="startOffset"
          from="50%"
