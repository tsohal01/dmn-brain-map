...
const STATE_FC = [
  ...
  { edge: "PCC\u2013HPC", hc: "Increased (Chen 2020, subsystem-level) \u26A0", mdd: "Increased \u2014 inferred from HC \u26A0" },
  { edge: "PCC\u2013dMPFC", hc: "Decreased (Chen 2020, subsystem-level) \u26A0", mdd: "Decreased \u2014 partially inferred \u26A0" },
  { edge: "dMPFC\u2013TPJ", hc: "Normal", mdd: "Decreased \u2014 untested prediction \u26A0" },
  { edge: "HPC\u2013PHC", hc: "Decreased (Chen 2020, subsystem-level) \u26A0", mdd: "Decreased \u2014 inferred from HC \u26A0" },
...
const TRAIT_FC = [
  ...
  { edge: "dMPFC\u2013TempP", hc: "\u2014", mdd: "Increased (Zhu 2017; RSQ-correlated) \u2713" },
  { edge: "TPJ\u2013LTC", hc: "\u2014", mdd: "Increased (Zhu 2017) \u2713" },
  { edge: "LTC\u2013PHC", hc: "\u2014", mdd: "Increased (Zhu 2017; RSQ-correlated) \u2713" },
  { edge: "TPJ\u2013PHC", hc: "\u2014", mdd: "Increased (Zhu 2017) \u2713" },
  { edge: "TempP\u2013PHC", hc: "\u2014", mdd: "Increased (Zhu 2017) \u2713" },
...
const RegionNode = ({ id, x, y, subsystem, isActive, isTonic, isDimmed, isHovered, isLateral, onHover, onLeave, onClick, isPinned }) => {
  ...
  return (
    <g onMouseEnter={() => onHover(id)} onMouseLeave={onLeave}
      ...
      {isPinned && !isDimmed && (
        <circle cx={x} cy={y} r={r + 5} fill="none" stroke="#fff" strokeWidth="1" opacity="0.4" strokeDasharray="3,2" style={{ pointerEvents: "none" }} />
      )}
      {isTonic && !isActive && (
...
const DetailBox = ({ hovR, hovRId, hovE, data, tab, side, isPinned }) => {
    ...
    return (
      <div>
        <div style={{ display: "flex", alignItems: "left", gap: 8 }}>
          ...
          {isPinned && <span style={{ fontSize: 9, color: "#22C55E", fontFamily: "monospace", padding: "1px 5px", background: "#22C55E15", borderRadius: 3 }}>{"\uD83D\uDCCC"} Pinned</span>}
        </div>
  ...
  return <div style={{ fontSize: 10.5, color: "#4A5A6A", fontStyle: "italic" }}>Hover or click a region / connection for sourced details</div>;
};
    ...
    <div style={{ flex: 1, minWidth: 290, background: "linear-gradient(168deg, #0D1520 0%, #111C2C 100%)", borderRadius: 14, border: "1px solid #1E2D42", padding: "14px 12px 12px" }}>
      ...
      <svg viewBox="0 0 370 290" style={{ width: "100%", height: "auto", maxHeight: 280 }}>
        ...
        {data.connections.map((c, i) => {
          ...
          return <CurvedConn key={i} edgeKey={key} from={c.from} to={c.to} type={c.type}
            isHovered={hovE === key} isPinned={pinnedE === key}
        ...
        {Object.entries(REGIONS).map(([id, reg]) => (
          <RegionNode key={id} id={id} x={reg.x} y={reg.y} subsystem={reg.sub}
            ...
            isDimmed={!!reg.dimmed} isHovered={hovR === id} isPinned={pinnedR === id}
      ...
      <div style={{ textAlign: "left", minHeight: 76, padding: "10px 12px", background: "#0A1018", borderRadius: 8, marginTop: 4, border: `1px solid ${isPinned ? "#22C55E33" : "#1A2940"}` }}>
...
const HierarchyTab = () => {
  ...
  const isZoomed = !!zoomedSub;

  const getZoomTransform = () => {
    if (!zoomedSub) return { transform: "translate(0px, 0px) scale(1)", transformOrigin: "0 0" };
    let tcx, tcy, tr;
  ...
  const handleSubClick = (subId, e) => {
    ...
    if (zoomedSub === subId) { setZoomedSub(null); return; }
    setZoomedSub(subId);
    setHov(null);
  };

  const handleBgClick = () => {
    if (isZoomed) { setZoomedSub(null); setHov(null); }
  };

  /* ── MODE EDGE DEFINITIONS ── */
  const MODE_EDGES = {
    resting: [
      { from: "PCC", to: "aMPFC", status: "normal" }, { from: "PCC", to: "dMPFC", status: "normal" },
      { from: "PCC", to: "HPC", status: "normal" }, { from: "dMPFC", to: "TPJ", status: "normal" },
      { from: "dMPFC", to: "LTC", status: "normal" }, { from: "dMPFC", to: "TempP", status: "normal" },
      { from: "HPC", to: "PHC", status: "normal" }, { from: "TPJ", to: "LTC", status: "normal" },
    ],
    mdd: [
      { from: "PCC", to: "aMPFC", status: "normal" }, { from: "PCC", to: "dMPFC", status: "reduced" },
      { from: "PCC", to: "sgACC", status: "hyper" }, { from: "dMPFC", to: "TempP", status: "hyper" },
      { from: "TPJ", to: "LTC", status: "hyper" }, { from: "LTC", to: "PHC", status: "hyper" },
      { from: "TPJ", to: "PHC", status: "hyper" }, { from: "TempP", to: "PHC", status: "hyper" },
      { from: "dMPFC", to: "TPJ", status: "normal" }, { from: "HPC", to: "PHC", status: "normal" },
    ],
    hc: [
      { from: "PCC", to: "aMPFC", status: "normal" }, { from: "PCC", to: "dMPFC", status: "normal" },
      { from: "PCC", to: "HPC", status: "normal" }, { from: "dMPFC", to: "TPJ", status: "normal" },
  ...
  const spotlightNodes = (() => {
    ...
    const s = new Set();
    (MODE_EDGES[mode] || []).forEach(e => { if (e.status !== "normal") { s.add(e.from); s.add(e.to); } });
    return s;
  })();
  const activeNodes = (() => {
    if (!mode) return new Set();
  ...
  /* ── Inner arc path (counterclockwise so text reads inside) ── */
  const arcPathInner = (ocx, ocy, r) => {
    return `M ${ocx + r} ${ocy} A ${r} ${r} 0 1 0 ${ocx - r} ${ocy} A ${r} ${r} 0 1 0 ${ocx + r} ${ocy}`;
  };
  ...
  const renderSubWords = (subId, sCx, sCy) => {
    ...
    const isThisZoomed = zoomedSub === subId;
    return words.map((w, i) => {
      const wx = sCx + w.dx;
      const wy = sCy + w.dy;
      const isHighlighted = isThisZoomed && hov === w.nearNode;
      ...
      return (
        <g key={`word-${subId}-${i}`}>
          <animateTransform attributeName="transform" type="translate"
            ...
            dur={`${dur.toFixed(1)}s`} repeatCount="indefinite" />
          <text x={wx} y={wy} textAnchor="middle" dominantBaseline="central"
            fill={isHighlighted ? SUB[subId].color : "#7A8A9A"}
            fontSize={w.size} fontFamily="'IBM Plex Sans', sans-serif"
            fontWeight={isHighlighted ? 700 : w.size >= 13 ? 600 : 400}
            opacity={isThisZoomed ? (isHighlighted ? 0.85 : 0.32) : 0}
            style={{
              pointerEvents: "none",
              transition: isThisZoomed
  ...
  const renderNode = (id, np, color, isRsp = false) => {
    ...
    /* Size scaling based on mode activity */
    let baseR = nodeR;
    let nodeOpacity = 1;
    if (mode) {
      if (activity === "active") {
    ...
    const scl = isH ? 1.12 : 1;
    return (
      <g key={id}>
        <animateTransform attributeName="transform" type="translate" values={bobVals[id]} dur={bobDur[id]} repeatCount="indefinite" />
        {isSpotlit && (
          <circle cx={np.cx} cy={np.cy} r={baseR + 8} fill={modeColors[mode] + "15"}
            stroke={modeColors[mode] + "33"} strokeWidth={1} filter="url(#spotGlow)"
            style={{ pointerEvents: "none" }} />
        )}
        {/* Active pulse ring for mode-active nodes */}
        {mode && activity === "active" && (
          <circle cx={np.cx} cy={np.cy} r={baseR + 3} fill="none"
            stroke={modeColors[mode]} strokeWidth={1.2} opacity={0}
            style={{ pointerEvents: "none" }}>
            <animate attributeName="r" values={`${baseR + 1};${baseR + 10}`} dur="2.5s" repeatCount="indefinite" />
        ...
        <g style={{ cursor: "pointer", transformOrigin: `${np.cx}px ${np.cy}px`, transform: `scale(${scl})`, transition: "transform 0.25s ease" }}
          ...
          <circle cx={np.cx} cy={np.cy} r={baseR}
            fill={isModeActive ? color + "55" : isH ? color + "44" : isRsp ? color + "22" : "#0D1520"}
            stroke={isH ? "#fff" : color + (isRsp ? "88" : "77")}
          ...
          <text x={np.cx} y={np.cy + 1} textAnchor="middle" dominantBaseline="central"
            ...
            fontWeight={700}
            opacity={nodeOpacity}
            fontFamily="'IBM Plex Mono', monospace" style={{ pointerEvents: "none", transition: "all 0.4s ease" }}>{id}</text>
        </g>
      </g>
    );
  };
  ...
  const renderInfoPanel = () => {
    ...
    return (
      <div style={{
        ...
        padding: "16px 18px", background: "linear-gradient(135deg, #0D1520EE 0%, #111C2CEE 100%)",
        borderRadius: "12px 0 0 12px", border: `1px solid ${zoomedColor}33`,
        borderRight: `2px solid ${zoomedColor}22`,
        backdropFilter: "blur(8px)",
        opacity: isZoomed ? 1 : 0,
        transform: isZoomed ? "translateX(0)" : "translateX(-30px)",
        transition: "opacity 0.4s ease, transform 0.4s ease",
        pointerEvents: isZoomed ? "auto" : "none",
        zIndex: 10, overflowY: "auto",
        boxSizing: "border-box",
      }}>
        <button onClick={() => { setZoomedSub(null); setHov(null); }}
          style={{
        ...
        {hovNodeData ? (
          <>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 8 }}>
              <div style={{ width: 10, height: 10, borderRadius: "50%", background: hovNodeData.color || zoomedColor, flexShrink: 0 }} />
              <span style={{ fontSize: 14, fontWeight: 700, color: hovNodeData.color || "#D0E0F0", fontFamily: "'IBM Plex Sans', sans-serif" }}>
                {hovNodeData.full || hovNodeData.label}
              </span>
            </div>
    ...
    <div style={{ padding: "0 8px" }}>
      <div style={{ textAlign: "center", marginBottom: 12 }}>
        ...
        <p style={{ fontSize: 10.5, color: "#5A7A9A", margin: 0, fontFamily: "monospace" }}>
          ...
          &nbsp;&nbsp;|&nbsp;&nbsp;Data provided by Andrews-Hanna 2010 & 2012 publications
        </p>
      </div>
      ...
      <div style={{ position: "relative", maxWidth: 820, margin: "0 auto" }}>
        ...
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", display: "block", overflow: "hidden", borderRadius: 12 }}
          ...
          <defs>
            ...
            <path id="arc-core-txt" d={arcPath(cx, cy, coreR + 5, true)} fill="none" />
          ...
          <g style={{
            ...
            <text fontSize={11.5} fontFamily="monospace" fontWeight={700} letterSpacing="0.24em"
              fill="#5A7A9A" opacity={isZoomed ? 0 : 1} style={{ pointerEvents: "none", transition: "opacity 0.3s" }}>
            ...
            {mode && (
              <g key={`mode-${mode}`}>
                {(MODE_EDGES[mode] || []).map((edge, i) => {
                  const d = edgePath(edge.from, edge.to);
                  const st = edgeStyle(edge.status);
                  return (
                    <g key={`${mode}-${edge.from}-${edge.to}-${edge.status}`}>
                      ...
                      <circle r="2" fill={st.color} opacity="0.85" filter="url(#synGlow)">
                        <animateMotion dur={edge.status === "hyper" ? "0.6s" : "1s"}
                          repeatCount="indefinite" path={d} begin={`${i * 0.1}s`} />
                      </circle>
                    </g>
                  );
                })}
            ...
            {(() => {
              ...
              return (
                <g opacity={isOtherZoomed ? 0.12 : 1} style={{ transition: "opacity 0.4s" }}>
                  ...
                  {!isZoomed && (hov === "core" ? (
                    <text x={cx} y={cy - coreR + 27} textAnchor="middle"
                  ...
                  {renderSubWords("core", cx, cy)}
                  {["PCC", "aMPFC"].map(id => renderNode(id, nodePos[id], SUB.core.color))}
                </g>
              );
            })()}
            ...
            {Object.entries(subCenters).map(([subId, sc]) => {
              ...
              const isThisZoomed = zoomedSub === subId;
              const isOtherZoomed = isZoomed && !isThisZoomed;
              return (
                <g key={subId} opacity={isOtherZoomed ? 0.12 : 1} style={{ transition: "opacity 0.4s" }}>
                  ...
                  <g style={{ cursor: "pointer" }}
                    ...
                    <circle cx={sc.cx} cy={sc.cy} r={sc.r}
                      fill={hov === subId && !isZoomed ? color + "0A" : "transparent"}
                      stroke={color} strokeWidth={isThisZoomed ? 1.8 : 1.4}
                      strokeDasharray="5,5" opacity={0.85}
                      style={{ transition: "fill 0.2s" }} />
                  </g>
                  {!isZoomed && (hov === subId ? (
                  ...
                  >
                    {subId === "mtl" ? (
                      <>
                        <tspan x={sc.cx} dy="0">MTL Subsystem</tspan>
                        <tspan x={sc.cx} dy="20">Memory &amp; Scenes</tspan>
                      </>
                    ) : (
                      sc.label
                    )}
                    ...
                    <text fontSize={subId === "aff" ? 8 : 9.5} fontFamily="monospace" fontWeight={700} letterSpacing="0.08em"
                      fill={color + "BB"}>
...
export default function DMNBrainMap() {
  ...
  return (
    <div style={{ background: "#080E18", color: "#C0D0E0", minHeight: "100vh", fontFamily: "'IBM Plex Sans', -apple-system, sans-serif", padding: "20px 16px" }}>
      ...
      <div style={{ display: "flex", textAlign: "center", justifyContent: "center" , gap: 4, background: "#0A1018", borderRadius: 10, padding: 4, maxWidth: 560, margin: "12px auto" }}>
      ...
      <>
      <div style={{ textAlign: "center", marginBottom: 12, textAlign: "center", justifyContent: "center" }}>
