...
const RESOURCE_FLOW = {
  flows: [
    { from: "PCC", to: "HPC", type: "reference", label: "core↔MTL ↑", status: "established",
      findingKey: "resource.flow-core-mtl",
      cite: "Chen 2020",
      note: "Also visible in State overlay (subsystem level)" },
    { from: "PCC", to: "sgACC", type: "reference", label: null, status: "established",
      findingKey: "resource.flow-pcc-sgacc",
      cite: "Replicated literature",
      note: "Already shown in State + Trait overlays" },
    { from: "PCC", to: "dMPFC", type: "reference", label: "core↔dMPFC ↓", status: "established",
      findingKey: "resource.flow-pcc-dmpfc",
      cite: "Chen 2020 + Zhu 2017",
      note: "Also visible in Trait overlay (PCC–dMPFC reduced)" },
    { from: "dMPFC", to: "TPJ", type: "gap", label: "untested prediction", status: "gap",
      findingKey: "resource.flow-gap-tpj",
      cite: "Author's synthesis",
      note: "No concurrent ToM + rumination study exists" },
  ],
  citations: {
  ...
  annotations: [
    { text: "integrates self + mentalizing (Moran 2013)", near: "dMPFC", dx: -130, dy: -84, size: 9.5, tone: "contested" },
    { text: "mentalizing displaced?", near: "TPJ", dx: 104, dy: -42, size: 10, tone: "tom" },
    { text: "author's synthesis — not a single-paper claim", near: "PCC", dx: 124, dy: 76, size: 8, tone: "caveat" },
  ],
  nodeRoles: {
    PCC: "contested", aMPFC: "contested", dMPFC: "bridge",
    TPJ: "starved", LTC: "starved", TempP: "starved",
    HPC: "fed", PHC: "fed", Rsp: "dormant", sgACC: "fed",
  },
  subsystemRoles: { core: "contested", dm: "draining", mtl: "filling", aff: "intensified" },
};
...
const FINDING_TO_PAPERS = {
  ...
  "resource.flow-gap-tpj": ["moran2013", "bora_berk2016", "nestor2022", "kim2023"],
  ...
  const renderNode = (id, np, color, isRsp = false) => {
    ...
    if (resourceFlowActive) {
      if (rfRole === "fed") {
        baseR = nodeR + 5;
        nodeOpacity = 1;
      } else if (rfRole === "contested") {
        baseR = nodeR + 4;
        nodeOpacity = 1;
      } else if (rfRole === "bridge") {
        baseR = nodeR + 7;
        nodeOpacity = 1;
      } else if (rfRole === "starved") {
        baseR = nodeR - 2;
        nodeOpacity = 0.42;
      } else {
        baseR = nodeR - 4;
    ...
    const rfColor =
      rfRole === "fed" ? "#E8A838" :
      rfRole === "contested" ? "#D4845A" :
      rfRole === "bridge" ? "#C17AE0" :
      rfRole === "starved" ? "#5A8FB8" :
      null;
    ...
    return (
      ...
      >
        <g>
          ...
          {/* ── RESOURCE FLOW: bridge node split-color oscillating ring ── */}
          {resourceFlowActive && rfRole === "bridge" && !isMuted && (
            <>
              <circle cx={0} cy={0} r={baseR + 9} fill="none"
                stroke="#E8A838" strokeWidth="1.5"
                strokeDasharray={`${Math.PI * (baseR + 9)} ${Math.PI * (baseR + 9)}`}
                strokeDashoffset="0" opacity="0.52">
                <animate attributeName="opacity" values="0.62;0.18;0.62" dur="2.6s" repeatCount="indefinite" />
              </circle>
              <circle cx={0} cy={0} r={baseR + 9} fill="none"
                stroke="#5A8FB8" strokeWidth="1.5"
                strokeDasharray={`${Math.PI * (baseR + 9)} ${Math.PI * (baseR + 9)}`}
                strokeDashoffset={`${Math.PI * (baseR + 9)}`} opacity="0.52">
                <animate attributeName="opacity" values="0.18;0.62;0.18" dur="2.6s" repeatCount="indefinite" />
              </circle>
            </>
          )}
          {/* ── RESOURCE FLOW: fed nodes warm pulse ── */}
          {resourceFlowActive && rfRole === "fed" && !isMuted && (
            <circle cx={0} cy={0} r={baseR + 3} fill="none"
              stroke="#E8A838" strokeWidth={0.8} opacity={0}>
              <animate attributeName="r" values={`${baseR + 1};${baseR + 11}`} dur="3.8s" repeatCount="indefinite" />
              <animate attributeName="opacity" values="0.3;0" dur="3.8s" repeatCount="indefinite" />
            </circle>
          )}
          {/* ── RESOURCE FLOW: contested nodes warm/cool oscillation ── */}
          {resourceFlowActive && rfRole === "contested" && !isMuted && (
            <circle cx={0} cy={0} r={baseR + 4} fill="none"
              stroke="#D4845A" strokeWidth={0.7} opacity={0}>
              <animate attributeName="r" values={`${baseR + 2};${baseR + 10}`} dur="4.2s" repeatCount="indefinite" />
              <animate attributeName="opacity" values="0.22;0" dur="4.2s" repeatCount="indefinite" />
            </circle>
          )}
          <g
            style={{ cursor: "pointer" }}
      ...
      <GlassCard variant="elevated" style={{ textAlign: "center", marginBottom: 12, padding: "10px 12px" }}>
        ...
        <p style={{ fontSize: 10.5, color: "#5A7A9A", margin: 0, fontFamily: "monospace" }}>
	          {isZoomed
	            ...
	            : resourceFlowActive
	            ? "Resource Flow lens — context edges (already in overlays) + gap prediction: dMPFC→TPJ mentalizing during rumination (untested)"
    ...
    return (
      <g key={`sub-${overlayTop}-${mode}-${e.from}-${e.to}-${i}`}>
        ...
        {/* traveling dots removed from subsystem arcs — they compete with node-level edges */}
            ...
            {/* ── RESOURCE FLOW: reference context + gap prediction ── */}
            {resourceFlowActive && (
              <g className="resource-flow-layer">
                {/* Reference edges — static dashed, no particles (already shown in state/trait) */}
                {RESOURCE_FLOW.flows.filter(f => f.type === "reference").map((f, i) => {
                  const d = edgePath(f.from, f.to, displayNodePos);
                  if (!d) return null;
                  return (
                    <g key={`rf-ref-${i}`} opacity={0.35}>
                      <path d={d} fill="none" stroke="#5A7A9A" strokeWidth={1.2}
                ...
                {(() => {
                  const rfRefs = RESOURCE_FLOW.flows.filter((f) => f.type === "reference" && f.label);
                  const panelH = 34 + rfRefs.length * 11;
                  return (
                    <g transform="translate(24 156)">
                      ...
                      <text
                        ...
                        fontWeight={700}
                      >
                        REFERENCE CONTEXT (already shown in state/trait)
                      </text>
                      {rfRefs.map((f, i) => (
                        <g key={`rf-ref-group-${i}`}>
                          <text
                            x={10}
                            y={28 + i * 11}
                            fill="#7FA3C8"
                ...
                {/* Gap edge — animated, prominent, the lens's unique contribution */}
                {RESOURCE_FLOW.flows.filter(f => f.type === "gap").map((f, i) => {
                  const d = edgePath(f.from, f.to, displayNodePos);
                  if (!d) return null;
                  const pathId = `rf-gap-${i}`;
                  return (
                    <g key={`rf-gap-${i}`}>
                      {/* Pulsing glow */}
                      <path d={d} fill="none" stroke="#8B68B8" strokeWidth={6}
                        strokeLinecap="round" opacity={0.06}>
                        <animate attributeName="opacity" values="0.03;0.09;0.03" dur="3s" repeatCount="indefinite" />
                      </path>
                      {/* Main animated line */}
                      <path id={pathId} d={d} fill="none" stroke="#8B68B8"
                        strokeWidth={2.2} strokeDasharray="3,6" opacity={0.6} strokeLinecap="round">
                        <animate attributeName="stroke-dashoffset" from="0" to="-42" dur="3.6s" repeatCount="indefinite" />
                      </path>
                      {/* Moving particles */}
                      {[0, 0.33, 0.66].map((offset, pi) => (
                        <circle key={`gp-${i}-${pi}`} r={1.8} fill="#A888D4" opacity={0.45}>
                          <animateMotion dur="5s" begin={`${offset * 5}s`} repeatCount="indefinite" rotate="auto">
                            <mpath href={`#${pathId}`} />
                          </animateMotion>
                          <animate attributeName="opacity" values="0;0.5;0.5;0" dur="5s"
                            begin={`${offset * 5}s`} repeatCount="indefinite" />
                        </circle>
                      ))}
                      {/* Gap question mark */}
                      <text
                        x={(displayNodePos[f.from].cx + displayNodePos[f.to].cx) / 2 + 2}
                        y={(displayNodePos[f.from].cy + displayNodePos[f.to].cy) / 2 + 2}
                        textAnchor="middle" dominantBaseline="central"
                        fill="#8B68B8" fontSize="16" fontWeight={700}
                      ...
                      <text
                        x={(displayNodePos[f.from].cx + displayNodePos[f.to].cx) / 2}
                        y={(displayNodePos[f.from].cy + displayNodePos[f.to].cy) / 2 - 12}
                        textAnchor="middle" fill="#A888D4" fontSize="8.5"
                        fontFamily="'IBM Plex Mono', monospace" fontStyle="italic" opacity={0.72}>
                        untested prediction — your study fills this
                      </text>
                    </g>
                  );
                })}

                {/* Floating annotations (reduced to 3) */}
                {RESOURCE_FLOW.annotations.map((a, i) => {
                  const np = displayNodePos[a.near];
                  if (!np) return null;
                  const toneColor =
                    a.tone === "tom" ? "#5A8FB8" :
                    a.tone === "contested" ? "#C17AE0" :
                    a.tone === "caveat" ? "#7A8A9A" : "#9FC2E6";
                  return (
                    <text key={`rfa-${i}`}
                      x={np.cx + a.dx} y={np.cy + a.dy}
                      textAnchor="middle" fill={toneColor} fontSize={a.size}
                      fontFamily="'IBM Plex Mono', monospace"
                      fontStyle={a.tone === "caveat" ? "italic" : "normal"}
                      opacity={a.tone === "caveat" ? 0.52 : 0.72}
                      style={{ pointerEvents: "none" }}>
                      {a.text}
                    </text>
                  );
                ...
                {/* Legend — simplified */}
                <g transform="translate(548 172)">
                  <rect x={0} y={0} width={232} height={52} rx={8}
                    fill="rgba(7,12,19,0.78)" stroke="rgba(97,123,151,0.35)" />
                  <line x1={10} y1={14} x2={46} y2={14} stroke="#5A7A9A" strokeWidth={1.2} strokeDasharray="4,6" opacity={0.5} />
                  <text x={52} y={17} fill="#6A8AAA" fontSize="8.5" fontFamily="'IBM Plex Mono', monospace" opacity={0.7}>context (also in state/trait)</text>
                  <line x1={10} y1={30} x2={46} y2={30} stroke="#8B68B8" strokeWidth={2.2} strokeDasharray="3,6" />
                  <text x={52} y={33} fill="#A888D4" fontSize="8.5" fontFamily="'IBM Plex Mono', monospace">gap prediction (untested)</text>
                  <text x={116} y={46} textAnchor="middle" fill="#6A7A8A" fontSize="7.5" fontFamily="'IBM Plex Mono', monospace" fontStyle="italic">author's synthesis</text>
                </g>

                {/* Central narrative — simplified */}
                <g transform="translate(206 236)">
                  <rect x={0} y={0} width={392} height={28} rx={10}
                    fill="rgba(7, 12, 19, 0.78)" stroke="rgba(193, 122, 224, 0.28)" strokeDasharray="6,5" />
                  <text x={196} y={17} textAnchor="middle" fill="#9A88C0" fontSize="9"
                    fontFamily="'IBM Plex Mono', monospace" fontStyle="italic">
                    What happens to mentalizing when the dMPFC is reconfigured? No study has tested this.
                  </text>
                </g>
              </g>
            )}
                ...
                <g key={subId} opacity={isHiddenByNodeFocus ? 0 : (isOtherZoomed ? 0.12 : 1)} style={{ transition: "opacity 0.4s" }}>
                  <g style={{ opacity: lensActive ? 0 : 1, transition: "opacity 0.55s ease", pointerEvents: "auto" }}>
                    ...
                    <g style={{ cursor: "pointer" }}
                      ...
                      {(() => {
                        const rfSubRole = resourceFlowActive ? (RESOURCE_FLOW.subsystemRoles[subId] || "dormant") : null;
                        const rfStroke = rfSubRole === "filling" ? "#E8A838" : rfSubRole === "draining" ? "#5A8FB8" : rfSubRole === "intensified" ? "#E8A838" : color;
                        const rfFill = rfSubRole === "filling" ? "#E8A83808" : rfSubRole === "draining" ? "#5A8FB805" : rfSubRole === "intensified" ? "#E8A83806" : "transparent";
                        const rfDash = rfSubRole === "draining" ? "3,6" : rfSubRole === "filling" ? "8,3" : "5,5";
                        const rfSW = rfSubRole === "draining" ? 0.9 : rfSubRole === "filling" ? 2.2 : 1.4;
                        return (
                      <circle
                        cx={sc.cx}
                        cy={sc.cy}
        ...
        {!isZoomed && (
          <GlassCard variant="soft" style={{ width: 248, minWidth: 220, padding: "10px 10px 12px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <div style={{ fontSize: 9.5, color: UI.color.textMuted, lineHeight: 1.5, fontFamily: "'IBM Plex Mono', monospace" }}>
                Resource Flow Lens visualizes the central gap: established subsystem reconfiguration (Core↔MTL up, Core↔dMPFC down) is shown as context, while the untested downstream prediction (dMPFC→TPJ mentalizing displacement) is highlighted. Citation links go to specific paper passages.
              </div>
            </div>
          </GlassCard>
        )}
          ...
          {mode && !hovNode ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={{ fontSize: 12.5, color: "#D4A8F0", lineHeight: 1.6 }}>
                Resource Flow lens: the established subsystem shifts are shown as context (already visible in State/Trait overlays). The animated edge is this study's gap — what happens to <span style={{ color: "#A888D4", fontWeight: 700 }}>dMPFC→TPJ mentalizing</span> when rumination reconfigures coupling?
              </div>
              <div style={{ fontSize: 11, color: "#8A9AB0", lineHeight: 1.6, borderLeft: "2px solid #5A7A9A44", paddingLeft: 10 }}>
                <span style={{ color: "#AFC7E3", fontWeight: 600 }}>What's established:</span> During rumination, Core↔MTL coupling increases and Core↔dMPFC coupling decreases (Chen 2020). The dMPFC integrates both self-referential and mentalizing functions (Moran 2013, Levorsen 2025), with self-processing as primary driver. These edges are already displayed in your State and Trait overlays.
              </div>
              <div style={{ fontSize: 11, color: "#8A9AB0", lineHeight: 1.6, borderLeft: "2px solid #8B68B844", paddingLeft: 10 }}>
                <span style={{ color: "#A888D4", fontWeight: 600 }}>What's untested:</span> If dMPFC coupling is reconfigured during rumination, what happens to its downstream connection to TPJ (the other-focused mentalizing node)? No study has administered a ToM task during active rumination. That's the gap your concurrent design would fill.
              </div>
