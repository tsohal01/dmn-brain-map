...
const VennHero = ({ opacity, scale }) => {
  const [hovOv, setHovOv] = useState(null);
  const d = VENN_DATA;

  if (opacity < 0.01) return null;

  // ── Detail text for inside each circle (non-overlap zones) ──
  const circleDetails = {
    rumination: [
  ...
  return (
    <div style={{
      ...
      {/* ── VENN SVG ── 
          Bigger viewBox (700x560) to give circles room.
          Circles styled to match subsystem circles:
            - strokeDasharray="5,5" (same as subsystem boundary rings)
            - strokeWidth 1.4 (default subsystem weight)
            - opacity 0.85 (subsystem default)
            - transparent fill with faint color on hover (color + "0A")  */}
      <svg viewBox="0 0 700 560" style={{ width: "100%", maxWidth: 680, height: "auto" }}>
        <defs>
          {d.circles.map((c) => (
            <radialGradient key={`vg-${c.id}`} id={`venn-grad-${c.id}`}>
              <stop offset="0%" stopColor={c.color} stopOpacity="0.10" />
              <stop offset="60%" stopColor={c.color} stopOpacity="0.05" />
        ...
        {/* ── Circle boundaries ──
            Matching subsystem style: dashed stroke, same weight/opacity */}
        {d.circles.map((c) => (
          <circle
            key={c.id}
            cx={c.cx} cy={c.cy} r={c.r}
            fill={`url(#venn-grad-${c.id})`}
            stroke={c.color}
            strokeWidth="1.4"
            strokeDasharray="5,5"
            opacity="0.85"
        ...
        {/* ── Circle titles + citations ── */}
        {d.circles.map((c) => {
          const lx = c.id === "mentalizing" ? c.cx - 48
                   : c.id === "depression"  ? c.cx + 48
                   : c.cx;
          const ly = c.id === "rumination" ? c.cy - 65 : c.cy + 58;
        ...
        {/* ── Detail text inside each circle ──
            Positioned in the non-overlapping region of each circle
            so they don't collide with overlap labels. */}
        {d.circles.map((c) => {
          const details = circleDetails[c.id] || [];
          // Offset text into the "pure" zone of each circle
          const tx = c.id === "mentalizing" ? c.cx - 62
                   : c.id === "depression"  ? c.cx + 62
                   : c.cx;
          const ty = c.id === "rumination" ? c.cy + 10 : c.cy - 18;
        ...
        {d.overlaps.map((ov) => (
          ...
          >
            ...
            />
            <text x={ov.x} y={ov.y - 3}
              textAnchor="middle"
              fill={hovOv === ov.id ? UI.color.textBase : UI.color.textMuted}
              fontSize="9"
              fontWeight="600"
              fontFamily="'IBM Plex Sans', sans-serif"
              style={{ transition: "fill 0.2s" }}
        ...
        {/* ── CENTER GAP ──
            Dashed ring to visually mark it as "empty / untested."
            This is the only circle with a dashed *different* pattern (4,3)
            to distinguish it from the subsystem-matching 5,5 on the main circles. */}
        <circle
          cx={d.center.cx} cy={d.center.cy} r={d.center.r}
          ...
          strokeOpacity="0.3"
        >
          <animate attributeName="strokeOpacity" values="0.3;0.15;0.3" dur="3.5s" repeatCount="indefinite" />
        </circle>
        <text x={d.center.cx} y={d.center.cy - 2} textAnchor="middle"
          fill="#fff" fontSize="10" fontWeight="700"
          fontFamily="'IBM Plex Mono', monospace"
          letterSpacing="0.1em" opacity="0.65">
          GAP
        </text>
        <text x={d.center.cx} y={d.center.cy + 12} textAnchor="middle"
          fill={UI.color.textQuiet} fontSize="7.5"
          fontFamily="'IBM Plex Mono', monospace">
          untested
        </text>
      </svg>
...
export default function DMNBrainMap() {
  ...
  const handleScroll = useCallback(() => {
    ...
    // Transition zone: 0.2vh of pure Venn, then 2.0vh of scrub
    const start = vh * 0.2;
    const end = vh * 2.2;
    const raw = (scrolledInto - start) / (end - start);
  ...
  // Phase A (0→0.5): Venn fades out, shrinks slightly
  const vennOpacity = 1 - scrollPhase(p, 0, 0.5);
  const vennScale = 1 - 0.06 * scrollPhase(p, 0, 0.55);
  const chevronOpacity = 1 - scrollPhase(p, 0, 0.35);

  // "Keep scrolling" visible only in the middle of transition,
  // disappears once you're past 92% OR haven't started yet
  const showContinue = scrollProgress >= 0.55 && scrollProgress < 0.92;

  // Skip-to-content: scrolls past the Venn zone
  const skipToContent = () => {
    if (scrollContainerRef.current) {
      const containerTop = scrollContainerRef.current.offsetTop;
      const containerHeight = scrollContainerRef.current.offsetHeight;
      window.scrollTo({ top: containerTop + containerHeight, behavior: "smooth" });
    }
  };
  ...
  return (
    <div style={{
      ...
      {/* ══════════════════════════════════════════════════════
          SECTION 1: VENN SCROLL ZONE
          ──────────────────────────────────────────────────────
          This is the intro. 250vh tall, with a sticky viewport
          inside. The Venn diagram fades as you scroll through.
          Once you exit the bottom, DMNBrainMapInner is waiting
          directly below as a normal page section.
          ══════════════════════════════════════════════════════ */}
      <div ref={scrollContainerRef} style={{ height: "250vh", position: "relative" }}>
        <div style={{
          position: "sticky",
          top: 0,
          height: "100vh",
          ...
          <div style={{
            ...
            alignItems: "center",
            padding: "20px 16px",
            pointerEvents: vennOpacity < 0.05 ? "none" : "auto",
          }}>
            <VennHero opacity={vennOpacity} scale={vennScale} />
          </div>

          {/* ── Scroll chevrons (right side) ── */}
          ...
          <button
            ...
            style={{
              ...
              fontFamily: "'IBM Plex Mono', monospace",
              cursor: "pointer",
              opacity: scrollProgress < 0.85 ? 0.7 : 0,
          ...
          <div style={{
            ...
            background: `linear-gradient(90deg, #4A90D9, #E8A838, #E05555)`,
            opacity: scrollProgress > 0.01 && scrollProgress < 0.92 ? 0.45 : 0,
            transition: "opacity 0.2s",
            zIndex: 5,
          }} />

          {/* ── "Keep scrolling" — only visible mid-transition, 
                 disappears before you reach the DMN section ── */}
          {showContinue && (
            <div style={{
              position: "absolute",
              bottom: 28,
              ...
              zIndex: 5,
              animation: "vennFadeInUp 0.5s ease-out",
      ...
      {/* ══════════════════════════════════════════════════════
          SECTION 2: DMN BRAIN MAP
          ──────────────────────────────────────────────────────
          Rendered as a normal page section BELOW the scroll
          container. No nesting inside sticky, no overflow trap.
          Full interactivity: tabs, toggles, hover, zoom — all
