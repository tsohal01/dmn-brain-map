...
const HierarchyTab = () => {
  ...
  const W = 660, H = 580;
  const cx = W / 2, cy = 290;
  const coreR = 44;                // radius of the core grouping ellipse
  const subRingR = 140;            // subsystem ring radius from center
  const regRingR = 240;            // region ring radius from center
  ...
  /* ── core hubs at center ── */
  const coreGroup = TREE.children[0];
  const coreNodes = [
    { ...coreGroup.children[0], cx: cx - 30, cy: cy - 6 },   // PCC (left)
    { ...coreGroup.children[1], cx: cx + 30, cy: cy + 6 },    // aMPFC (right)
  ];

  /* ── subsystem angles: dm top, mtl bottom-left, aff bottom-right ── */
  const subAngleMap = { dm: -90, mtl: 150, aff: 30 };
  const fanSpread = 26; // degrees between sibling regions

  const subsystems = TREE.children.filter((s) => s.id !== "core");
  const subPositions = subsystems.map((s) => {
    const angle = subAngleMap[s.id];
    const pos = polar(angle, subRingR);
    const n = s.children.length;
    const regions = s.children.map((r, i) => {
      const offset = n === 1 ? 0 : (i - (n - 1) / 2) * fanSpread;
      const rp = polar(angle + offset, regRingR);
      return { ...r, cx: rp.x, cy: rp.y, parentAngle: angle, ownAngle: angle + offset };
    });
    return { ...s, cx: pos.x, cy: pos.y, angle, regions };
  });

  /* ── hover lookup across all nodes ── */
  const allNodes = [
    TREE,
    coreGroup,
    ...coreGroup.children,
    ...subsystems,
    ...subsystems.flatMap((s) => s.children),
  ];
  const hovNode = hov ? allNodes.find((n) => n.id === hov) : null;

  /* ── helper: curved connection line ── */
  const curvedPath = (x1, y1, x2, y2, bulge = 0.15) => {
  ...
  /* ── node dimensions ── */
  const subW = 136, subH = 44;
  const regW = 108, regH = 36;
    ...
    <div style={{ padding: "0 8px" }}>
      <div style={{ textAlign: "center", marginBottom: 16 }}>
        ...
        <p style={{ fontSize: 10.5, color: "#5A7A9A", margin: 0, fontFamily: "monospace" }}>
          Hover any node for explanation&nbsp;&nbsp;|&nbsp;&nbsp;Based on Andrews-Hanna 2010, 2012 parcellation
        </p>
      </div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", maxWidth: 700, display: "block", margin: "0 auto" }}>
        ...
        {subPositions.map((s) => (
          <path
            key={`core-${s.id}`}
            d={curvedPath(cx, cy, s.cx, s.cy, 0.08)}
            fill="none"
            stroke={s.color}
            strokeWidth={1.8}
            opacity={hov === s.id || hov === "core" ? 0.55 : 0.2}
            strokeLinecap="round"
            style={{ transition: "opacity 0.15s" }}
          />
        ))}

        {/* ── connection lines: subsystem → its regions ── */}
        {subPositions.map((s) =>
          s.regions.map((r) => (
            <line
              key={`sub-${r.id}`}
              ...
              stroke={s.color}
              strokeWidth={0.9}
              opacity={hov === r.id || hov === s.id ? 0.4 : 0.12}
              style={{ transition: "opacity 0.15s" }}
            />
          ))
        )}

        {/* ── core grouping ellipse (hoverable for group desc) ── */}
        <g style={{ cursor: "pointer" }}
          onMouseEnter={() => setHov("core")} onMouseLeave={() => setHov(null)}>
          <ellipse cx={cx} cy={cy} rx={coreR + 22} ry={coreR + 4}
            fill="#4A90D908" stroke={hov === "core" ? SUB.core.color + "55" : SUB.core.color + "30"}
            strokeWidth={hov === "core" ? 1.6 : 1.2} strokeDasharray="4,5"
            style={{ transition: "all 0.15s" }} />
          <text x={cx} y={cy + 34} textAnchor="middle" fill={hov === "core" ? SUB.core.color : "#3A4A5C"}
            fontSize={8} fontFamily="monospace" fontWeight={600} letterSpacing="0.1em"
            style={{ transition: "fill 0.15s" }}>CORE HUBS</text>
        </g>

        {/* ── CORE HUB NODES ── */}
        {coreNodes.map((n) => {
          const isH = hov === n.id || hov === "core";
          return (
            <g key={n.id} style={{ cursor: "pointer" }}
              onMouseEnter={() => setHov(n.id)} onMouseLeave={() => setHov(null)}>
              <circle cx={n.cx} cy={n.cy} r={22}
                fill={isH ? SUB.core.color + "22" : "#0D1520"}
                stroke={isH ? SUB.core.color : SUB.core.color + "66"}
                strokeWidth={isH ? 2 : 1.2}
                style={{ transition: "all 0.15s" }} />
              <text x={n.cx} y={n.cy} textAnchor="middle" dominantBaseline="central"
                fill={isH ? "#fff" : SUB.core.color} fontSize={10.5} fontWeight={700}
                fontFamily="'IBM Plex Mono', monospace">{n.label}</text>
            </g>
          );
        })}

        {/* ── SUBSYSTEM NODES (middle ring) ── */}
        {subPositions.map((s) => {
          const isH = hov === s.id;
          const tagline = s.id === "dm" ? "social cognition" : s.id === "mtl" ? "memory / scenes" : "emotional valence";
          return (
            <g key={s.id} style={{ cursor: "pointer" }}
              onMouseEnter={() => setHov(s.id)} onMouseLeave={() => setHov(null)}>
              <rect x={s.cx - subW / 2} y={s.cy - subH / 2} width={subW} height={subH} rx={8}
                fill={isH ? s.color + "1A" : "#0D1520"}
                stroke={isH ? s.color : s.color + "66"}
                strokeWidth={isH ? 1.8 : 1.2}
                style={{ transition: "all 0.15s" }} />
              <text x={s.cx} y={s.cy - 4} textAnchor="middle" fill={s.color}
                fontSize={10} fontWeight={700} fontFamily="'IBM Plex Sans', sans-serif">{s.label}</text>
              <text x={s.cx} y={s.cy + 11} textAnchor="middle" fill="#4A5A6A"
                fontSize={8} fontFamily="monospace">{tagline}</text>
            </g>
          );
        })}

        {/* ── REGION NODES (outer ring) ── */}
        {subPositions.map((s) =>
          s.regions.map((r) => {
            const isH = hov === r.id;
            const isRsp = r.id === "Rsp";
            return (
              <g key={r.id} style={{ cursor: "pointer" }}
                onMouseEnter={() => setHov(r.id)} onMouseLeave={() => setHov(null)}>
                <rect x={r.cx - regW / 2} y={r.cy - regH / 2} width={regW} height={regH} rx={6}
                  fill={isH ? s.color + "18" : "#0A1018"}
                  stroke={isH ? s.color : s.color + "44"}
                  strokeWidth={isH ? 1.5 : 0.9}
                  strokeDasharray={isRsp ? "3,3" : "none"}
                  style={{ transition: "all 0.15s" }} />
                <text x={r.cx} y={r.cy - 3} textAnchor="middle" fill={isRsp ? s.color + "99" : s.color}
                  fontSize={11} fontWeight={700} fontFamily="'IBM Plex Mono', monospace">{r.label}</text>
                <text x={r.cx} y={r.cy + 11} textAnchor="middle" fill={isRsp ? "#3A4A5C" : "#4A5A6A"}
                  fontSize={6.5} fontFamily="monospace">{r.full}</text>
              </g>
            );
          })
        )}
        ...
        {/* ── layer labels ── */}
        <text x={14} y={cy - subRingR + 2} textAnchor="start" fill="#2A3A4C" fontSize={7.5} fontFamily="monospace" letterSpacing="0.06em">SUBSYSTEM</text>
        <text x={14} y={cy - regRingR + 2} textAnchor="start" fill="#2A3A4C" fontSize={7.5} fontFamily="monospace" letterSpacing="0.06em">REGION</text>
      </svg>

      {/* ── DETAIL BOX ── */}
      <div style={{
        minHeight: 120, margin: "16px auto 0", maxWidth: 660, padding: "14px 18px",
        background: "#0D1520", borderRadius: 10, border: `1px solid ${hovNode ? (hovNode.color || "#1A2940") + "55" : "#1A2940"}`,
        ...
        {hovNode ? (
          ...
          <div style={{ fontSize: 11.5, color: "#4A5A6A", lineHeight: 1.7 }}>
            <span style={{ color: "#5A7A9A", fontWeight: 600 }}>Hover any node above.</span> The DMN isn't a flat list of regions — it has hierarchical structure. The <span style={{ color: SUB.core.color }}>core hubs</span> (PCC, aMPFC) sit at the center and integrate signals from specialized <span style={{ color: "#8899AA" }}>subsystems</span>: the <span style={{ color: SUB.dm.color }}>dorsomedial subsystem</span> handles social cognition and mentalizing, the <span style={{ color: SUB.mtl.color }}>MTL subsystem</span> handles episodic memory and scene construction, and the <span style={{ color: SUB.aff.color }}>affective node</span> (sgACC) interfaces with the network during emotional states. Rumination reconfigures the connections <em>between</em> these layers — that's what the State and Trait tabs show.
          </div>
        )}
      </div>
    </div>
